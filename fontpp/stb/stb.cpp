...
bool build(font_atlas* atlas)
{
    ...
    stbtt_PackBegin(&spc, nullptr, int(atlas->tex_width), tex_max, 0, int(atlas->tex_glyph_padding),
                    nullptr);
    ...
    for(auto& src_tmp : src_tmp_array)
    {
        ...
        for(int glyph_i = 0; glyph_i < src_tmp.glyphs_count; glyph_i++)
            if(src_tmp.rects[glyph_i].was_packed)
                atlas->tex_height = std::max<uint32_t>(atlas->tex_height, src_tmp.rects[glyph_i].y +
    ...
    for(size_t src_i = 0; src_i < src_tmp_array.size(); src_i++)
    {
        ...
        const float line_height =
            (unscaled_ascent - unscaled_descent + unscaled_line_gap) *
            font_scale;
