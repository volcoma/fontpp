...
const font_wchar* get_glyph_ranges_chinese_simplified_common()
{
	...
	static font_wchar full_ranges[countof(base_ranges) + countof(accumulative_offsets_from_0x4E00) * 2 + 1] =
		{0};
	if(!full_ranges[0])
	{
		memcpy(full_ranges, base_ranges, sizeof(base_ranges));
		UnpackAccumulativeOffsetsIntoRanges(0x4E00, accumulative_offsets_from_0x4E00,
											countof(accumulative_offsets_from_0x4E00),
...
void font_info::build_lookup_table()
{
	...
	for(size_t i = 0; i < glyphs.size(); i++)
	{
		...
		index_advance_x[codepoint] = glyphs[i].AdvanceX;
	...
	if(tab_glyph_found)
	{
		...
		index_advance_x[size_t(tab_glyph.Codepoint)] = tab_glyph.AdvanceX;
		index_lookup[size_t(tab_glyph.Codepoint)] = font_wchar(glyphs.size() - 1);
	}

	fallback_glyph = find_glyph_no_fallback(fallback_char);
	fallback_advance_x = fallback_glyph ? fallback_glyph->AdvanceX : 0.0f;
						  ...
						  float u1, float v1, float advance_x)
{
	...
	glyph.U1 = u1;
	glyph.V1 = v1;
	glyph.AdvanceX = advance_x + config_data->glyph_extra_spacing.x; // Bake spacing into AdvanceX
