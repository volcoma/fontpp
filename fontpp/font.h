...
//-----------------------------------------------------------------------------
struct font_info;   // Runtime data for a single font within a parent ImFontAtlas
struct font_atlas;  // Runtime data for multiple fonts, bake multiple fonts into a single texture, TTF/OTF
					// font loader
struct font_config; // Configuration data when adding a font or merging fonts
struct font_glyph;  // A single font glyph (code point + coordinates within in ImFontAtlas + offset)
...
// call BuildRanges(). This is essentially a tightly packed of vector of 64k booleans = 8KB storage.
struct font_glyph_ranges_builder
{
	std::vector<uint32_t> UsedChars; // Store 1-bit per Unicode code point (0=unused, 1=used)
	...
	inline void clear()
	{
		...
		UsedChars.resize(size_in_bytes / sizeof(uint32_t), 0);
	}
	...
	inline bool get_bit(int n) const
	{
		...
		return (UsedChars[size_t(off)] & mask) != 0;
	}
	...
	std::vector<font_wchar> build_ranges(); // Output new ranges
};
...
struct font_atlas
{
	...
	// Texture width desired by user before Build(). Must be a power-of-two. If have many
