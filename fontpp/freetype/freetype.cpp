...
bool build(FT_Library ft_library, font_atlas* atlas, unsigned int extra_flags)
{
    ...
    buf_bitmap_buffers.push_back((uint8_t*)std::malloc(chunk_size));
    ...
    const int tex_max = atlas->max_texture_size ? atlas->max_texture_size : 1024 * 8;
    ...
    const uint32_t surface_sqrt = uint32_t(std::sqrt(total_surface)) + 1;
    atlas->tex_height = 0;
    atlas->tex_width = estimate_width(tex_max, 512, surface_sqrt);
    ...
    stbrp_init_target(&pack_context, atlas->tex_width, tex_max, pack_nodes.data(), pack_nodes.size());
