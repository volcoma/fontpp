...
struct font_ft
{
	...
	void blit_glyph(const FT_Bitmap* ft_bitmap, uint8_t* dst, uint32_t dst_pitch,
					uint8_t* multiply_table = nullptr);
	~font_ft()
	{
		close();
	}
...
bool font_ft::init(FT_Library ft_library, const font_config& cfg, unsigned int extra_user_flags)
{
	...
	if(user_flags & rasterizer_flags::NoHinting)
		load_flags |= FT_LOAD_NO_HINTING;
	if(user_flags & rasterizer_flags::NoAutoHint)
		load_flags |= FT_LOAD_NO_AUTOHINT;
	if(user_flags & rasterizer_flags::ForceAutoHint)
		load_flags |= FT_LOAD_FORCE_AUTOHINT;
	if(user_flags & rasterizer_flags::LightHinting)
		load_flags |= FT_LOAD_TARGET_LIGHT;
	else if(user_flags & rasterizer_flags::MonoHinting)
...
void font_ft::set_pixel_height(int pixel_height)
{
	...
	info.line_spacing = (float)FT_CEIL(metrics.height);
	info.line_gap = (float)FT_CEIL(metrics.height - metrics.ascender + metrics.descender);
	info.max_advance_width = (float)FT_CEIL(metrics.max_advance);
}
...
static void unpack_bool_vector_to_flat_index_list(const bool_vector* in, std::vector<glyph_ft>* out)
{
...
bool build(FT_Library ft_library, font_atlas* atlas, unsigned int extra_flags)
{
		...
		for(const font_wchar* src_range = src_tmp.src_ranges; src_range[0] && src_range[1]; src_range += 2)
			for(int codepoint = src_range[0]; codepoint <= src_range[1]; codepoint++)
			{
				...
				uint32_t glyph_index = FT_Get_Char_Index(
					src_tmp.font.face, FT_ULong(codepoint)); // It is actually in the font?
	...
	std::vector<uint8_t*> buf_bitmap_buffers;
	buf_bitmap_buffers.push_back((uint8_t*)std::malloc(BITMAP_BUFFERS_CHUNK_SIZE));
	...
	for(int src_i = 0; src_i < src_tmp_array.size(); src_i++)
	{
		...
		for(int glyph_i = 0; glyph_i < src_tmp.glyphs_list.size(); glyph_i++)
		{
			...
			src_glyph.bitmap_data = buf_bitmap_buffers.back() + buf_bitmap_current_used_bytes;
	...
	const int num_nodes_for_packing_algorithm = atlas->tex_width - atlas->tex_glyph_padding;
	std::vector<stbrp_node> pack_nodes;
	pack_nodes.resize(num_nodes_for_packing_algorithm);
	stbrp_context pack_context;
	stbrp_init_target(&pack_context, atlas->tex_width, TEX_HEIGHT_MAX, pack_nodes.data(), pack_nodes.size());
	...
	for(size_t src_i = 0; src_i < src_tmp_array.size(); src_i++)
	{
		...
		for(int glyph_i = 0; glyph_i < src_tmp.glyphs_count; glyph_i++)
		{
			...
			glyph_info_ft& info = src_glyph.info;
			...
			size_t blit_dst_stride = (size_t)atlas->tex_width;
			uint8_t* blit_src = src_glyph.bitmap_data;
			uint8_t* blit_dst = atlas->tex_pixels_alpha8.data() + (ty * blit_dst_stride) + tx;
			...
			// Register glyph
			float x0 = info.offset_x + char_off_x;
			float y0 = info.offset_y + font_off_y;
			float x1 = x0 + info.width;
			float y1 = y0 + info.height;
			float u0 = (tx) / (float)atlas->tex_width;
			float v0 = (ty) / (float)atlas->tex_height;
			float u1 = (tx + info.width) / (float)atlas->tex_width;
			float v1 = (ty + info.height) / (float)atlas->tex_height;
	...
	// Cleanup
	for(auto& buf_bitmap_buffer : buf_bitmap_buffers)
		std::free(buf_bitmap_buffer);
...
bool build(font_atlas* atlas, unsigned int extra_flags)
{
	// FreeType memory management: https://www.freetype.org/freetype2/docs/design/design-4.html

	// https://www.freetype.org/freetype2/docs/reference/ft2-module_management.html#FT_New_Library
	FT_Library ft_library;
	FT_Error error = FT_New_Library(nullptr, &ft_library);
	if(error != 0)
		return false;
	...
	bool ret = build(ft_library, atlas, extra_flags);
