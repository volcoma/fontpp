...
bool build(FT_Library ft_library, font_atlas* atlas, unsigned int extra_flags)
{
	...
	const uint32_t tex_max = atlas->max_texture_size ? atlas->max_texture_size : 1024 * 8;
	...
	stbrp_init_target(&pack_context, int(atlas->tex_width), int(tex_max), pack_nodes.data(),
					  int(pack_nodes.size()));
